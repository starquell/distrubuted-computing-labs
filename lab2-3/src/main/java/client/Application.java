...
public class Application extends JFrame {
    ...
    private Application() {
        ...
        comboDepartment.addActionListener(e -> {
            currentDepartment = client.departmentFindByName((String) comboDepartment.getSelectedItem());
        ...
        comboPanel.add(new JLabel("DTO.Employee:"));
        comboPanel.add(comboEmployee);
        comboPanel.add(Box.createVerticalStrut(20));
        comboEmployee.addActionListener(e -> {
            currentEmployee = client.employeeFindByName((String) comboEmployee.getSelectedItem());
    ...
    private void save() {
        if (editMode) {
            if (departmentMode) {
                ...
                if (!client.departmentUpdate(currentDepartment)) {
                ...
                var department = client.departmentFindByName(textEmployeeDepartmentName.getText());
                if (department == null) {
                ...
                if (!client.employeeUpdate(currentEmployee)) {
                    JOptionPane.showMessageDialog(null, "Error: update failed!");
                }
            }
        } else {
            if (departmentMode) {
                ...
                if (!client.departmentInsert(department)) {
                ...
                Department department = client.departmentFindByName(textEmployeeDepartmentName.getText());
                if (department == null) {
                    JOptionPane.showMessageDialog(null, "Error: no such department!");
                    return;
                }
                ...
                if (!client.employeeInsert(employee)) {
                    JOptionPane.showMessageDialog(null, "Error: insertion failed!");
                    return;
                }
    ...
    private static void delete() {
        if (editMode) {
            if (departmentMode) {
                client.departmentDelete(currentDepartment);
                comboDepartment.removeItem(currentDepartment.getName());

            } else {
                client.employeeDelete(currentEmployee);
    ...
    private void fillComboBoxes() {
        ...
        var departments = client.departmentAll();
        var employees = client.employeeAll();
    ...
    private static void fillEmployeeFields() {
        ...
        Department department = client.departmentFindById(currentEmployee.getDepartmentId());
